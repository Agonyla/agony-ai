package com.agony.salesAgent.tools;
...
public class ChartGeneratorTool {
     ...
     * 生成折线图
     *
     * @param months     近多少个月的数据
     * @param regionName 大区名称
     * @param title      图表标题
     */
    @Tool("""
            生成销售趋势折线图的 ECharts JSON 数据。
            适用于：画折线图、趋势图。月度变化图等可视化需求。
            返回的 JSON 可直接用于前端 ECharts 渲染。
            """)
    public String generateLineChart(
        ...
        try {
            ...
            Long regionId = regionName != null && !regionName.isEmpty()
                    ? queryService.getRegionIdByName(regionName)
                    : null;

            List<MonthlyTrendDTO> data = queryService.queryMonthlyTrend(regionId, months);
            if (data.isEmpty()) {
                return "暂无数据，无法生成表格";
            }
            ...
            List<Number> amounts = data.stream()
                    .map(dto -> dto.totalAmount().longValue())
                    .map(v -> (Number) v)
                    .toList();
     ...
     */
    @Tool("""
            生成大区或销售员销售额对比的柱状图 ECharts JSON。
            适用于：画柱状图、对比图、排行榜图等可视化需求。
            """)
    public String generateBarChart(
        ...
        try {
            ...
            if ("region".equals(dimension)) {
                ...
                values = regions.stream().map(v -> (Number) v.totalAmount().longValue()).toList();
            } else {
                List<RepSalesDTO> reps = queryService.queryRepRanking(start, end, 10);
                names = reps.stream().map(RepSalesDTO::repName).toList();
                values = reps.stream().map(v -> (Number) v.totalAmount().longValue()).toList();
            }
     ...
     * 生成占比图
     *
     * @param dimension 饼图维度
     * @param startDate 查询开始日期
     * @param endDate   查询结束日期
     * @param title     图表标题
     */
    @Tool("""
            生成销售占比饼图的 ECharts JSON。
            适用于：画饼图、各部分占比份额分布等可视化需求。
            """)
    public String generatePieChart(
        ...
        try {
            ...
            if ("region".equals(dimension)) {
                List<RegionSalesDTO> regions = queryService.queryRegionRanking(start, end);
                pieData = regions.stream().map(region -> {
                    Map<String, Object> item = new LinkedHashMap<>();
                    item.put("name", region.regionName());
                    item.put("value", region.totalAmount().longValue());
                    return item;
                }).toList();
            } else {
